...
const Section2 = ({ clientId }: Props) => {
  if (window.sessionStorage.getItem("currency") == null) {
  ...
  useEffect(() => {
    ...
    window.sessionStorage.setItem("productPrice", String((parseFloat(amountRef.current.value) * rates[window.sessionStorage.getItem('currency')!]).toFixed(2)));
    ...
    if (amountRef.current) {
      ...
      if (!Number.isNaN(parseInt(amountRef.current.value))) {
        ...
        if (amount) {
         ...
         setTotal(String((parseFloat(amountRef.current.value) * rates[window.sessionStorage.getItem('currency')!]).toFixed(2)))
          setPrompt(`total = ${String((parseFloat(amountRef.current.value) * rates[window.sessionStorage.getItem('currency')!]).toFixed(2))}  ${window.sessionStorage.getItem('currency')}`);
         }
        }
      }
    }
  };
  const rateConversion = async()=>{
    
    if(amountRef.current){
   let r=  await fetchRate(window.sessionStorage.getItem('currency')!,String(parseFloat(amountRef.current.value)*0.001))
   return r
    }
  }
